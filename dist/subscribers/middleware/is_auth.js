    ...
    if (req.headers.authorization && req.headers.authorization.startsWith('Bearer')) {
        ...
        try {
            ...
            if (!account.sessionToken) {
                return next(new error_response_1.default('User authorization error: your session has ended, please login again', 401));
            }
            if (!account.isActive) {
                return next(new error_response_1.default('User authorization error: your account is inactive', 401));
            }
            req.user = account;
            return next();
        }
