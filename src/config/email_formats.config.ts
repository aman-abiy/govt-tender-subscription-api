...
import { COMPANY_PHONE_NUMBER_1, COMPANY_PHONE_NUMBER_2, DATE_OPTIONS, HOME_ROUTE, PACKAGES_ROUTE } from './statics.config';
import { getAlertTenders } from '../utils/alert_tenders';

export const welcomeEmail = (account: Account, verificationLink: URL) : MailOptions => {
        ...
        <head>
            ...
            <body>
                <div class="tender-body">
                    ...
                    <br/>
                    <p>You have successfully registered on <a href="${HOME_ROUTE}">alphatenders.com</a> and this is your confirmation email. Start your subscription now and get easy and organized access to tenders (ጨርታ) released in Ethiopia.</p> 
                    <div class="d-flex justify-content-center">
                        <p><a href="${verificationLink}" class="btn btn-email-confirmation text-center">Click here to confirm your email</a></p>
                    </div>
                    <p>To start your subscription, call us on <a href="tel:${COMPANY_PHONE_NUMBER_1}">${COMPANY_PHONE_NUMBER_1}</a> or <a href="tel:${COMPANY_PHONE_NUMBER_2}">${COMPANY_PHONE_NUMBER_2}</a>. Click <a href="${PACKAGES_ROUTE}">here</a> to view subscription packages we provide.</p>
                    <hr>
                    <p class="text-center">copyright &copy <a href="${HOME_ROUTE}">alphatenders.com</a> </p>
                    <hr>
                </div>              
            </body>
        </html>`
...
export const emailVerification = (account: Account, verificationLink: URL) : MailOptions => {
    var mailOptions : MailOptions = {
        from: 'Alpha Tenders <info@alphatenders.com>',
        to: account.email,
...
export const passwordResetEmail = (account: Account, passwordResetLink: URL) : MailOptions => {
    var mailOptions : MailOptions = {
        from: 'Alpha Tenders <info@alphatenders.com>',
        to: account.email,
...
export const alertEmail = async(account: Account) : Promise<[MailOptions, Array<Tender>, number]> => {
           ...
           <div class="col-sm-12">
                <div class="tender-card card" style="position: relative;
                    ...
                    <div class="card-body" style=" -webkit-box-flex: 1;
                        ...
                        <p class="tender-card-sub-texts" style="color: #525354; font-size: 13px;"><strong>Opening Date:</strong> <span class="values" style="margin-left: 10px;"> ${new Date(e.bidOpeningDate).toLocaleDateString("en-US", DATE_OPTIONS)} </span> </p>
                        <p class="tender-card-sub-texts" style="color: #525354; font-size: 13px;"><strong>Closing Date:</strong> <span class="values" style="margin-left: 10px;"> ${new Date(e.bidClosingDate).toLocaleDateString("en-US", DATE_OPTIONS)} </span> </p>
                        <p class="tender-card-sub-texts" style="color: #525354; font-size: 13px;"><strong>Published On:</strong> <span class="values" style="margin-left: 10px;"> ${new Date(e.publicationDate).toLocaleDateString("en-US", DATE_OPTIONS)} (${JSON.parse(JSON.stringify(e.tenderSources[0])).name.en})</span></p>
                    </div>
                </div>  
           </div>            
        </div> `
        ...
        html: `<html>
            ...
            <body style="background-color: #eff2f4;
                ...
                <div class="tender-body" style="margin-left: 12.5%;
                    ...
                    <hr style="border-color: rgba(0,0,0,.125);">
                    <h2 class="text-center alert-title" style="text-align: center!important; color: #1c2e59">Tender Alert - ${new Date(dates.endDate).toLocaleDateString("en-US", DATE_OPTIONS)}</h2>
                    <div class="alert alert-primary" style="color: #004085;
                        ...
                        border-radius: 0.25rem">
                        This is a summary of tenders posted on alphatenders.com on ${new Date(dates.endDate).toLocaleDateString("en-US", DATE_OPTIONS)} which is selected based on your category and region selection.
                    </div>
...
export const promoEmail = (email: string) : MailOptions => {
